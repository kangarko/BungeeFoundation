...
public abstract class SimpleSettings extends YamlStaticConfig {
	...
	/**
	 * What debug sections should we enable in {@link Debugger} ? When you call
	 * {@link Debugger#debug(String, String...)} those that are specified in
	 * this settings are logged into the console, otherwise no message is shown.
	 *
	 ...
	 * unless disabled in {@link Common#ADD_LOG_PREFIX} and
	 * {@link Common#ADD_TELL_PREFIX}.
	 *
	...
	// --------------------------------------------------------------------
	/**
	 * The lag threshold used for {@link LagCatcher} in milliseconds. Set to -1
	 * to disable.
	 *
	 * Typically for ChatControl:
	 *
	...
	// --------------------------------------------------------------------
	/**
	 * When processing regular expressions, limit executing to the specified
	 * time. This prevents server freeze/crash on malformed regex (loops).
	 *
	 * Regex_Timeout_Milis: 100
	 */
	public static Integer REGEX_TIMEOUT = 100;
	/**
	 * What commands should trigger the your main plugin command (separated by a
	 * comma ,)? See {@link SimplePlugin#getMainCommand()}
	 *
	...
	/**
	 * Antipiracy stuff for our protected software, leave empty to
	 * Serialization: ""
	 *
	 * // NOT MANDATORY //
	 */
	...
	/**
	 * Should we check for updates from SpigotMC and notify the console and
	 * users with permission?
	 *
	 * See {@link SimplePlugin#getUpdateCheck()} that you can make to return
	 * {@link SpigotUpdater} with your Spigot plugin ID.
	 *
	 * Typically for ChatControl:
	 *
	 * Notify_Updates: true
	...
	/**
	 * Should we enable inbuilt advertisements? ** We found out that users
	 * really hate this feature, you may want not to use this completelly ** **
	 * If you want to broadcast important messages regardless of this feature
	 * just implement your ** ** own Runnable that checks for a YAML file on
	 * your external server on plugin load. **
	 *
	 * Typically for ChatControl:
	 *
	...
	/**
	 * The configuration version number, found in the "Version" key in the
	 * file.,
	 */
	 ...
	 *
	 * You can place this class to {@link SimplePlugin#getSettingsClasses()} to
	...
	private static void init() {
		...
		if (isSet("Log_Lag_Over_Milis")) {
			...
			Valid.checkBoolean(
					LAG_THRESHOLD_MILLIS == -1 || LAG_THRESHOLD_MILLIS >= 0,
					"Log_Lag_Over_Milis must be either -1 to disable, 0 to log all or greater!");

			if (LAG_THRESHOLD_MILLIS == 0)
				Common.log(
						"&eLog_Lag_Over_Milis is 0, all performance is logged. Set to -1 to disable.");
		}
		...
		{ // Load localization
			...
			if (hasLocalization && !keySet)
				throw new FoException(
						"Since you have your Localization class you must set the 'Locale' key in "
								+ getFileName());
		...
		{ // Load main command alias
			...
			// if (SimplePlugin.getInstance().getMainCommand() != null &&
			// !keySet)
			// throw new FoException("Since you override getMainCommand in your
			// main plugin class you must set the 'Command_Aliases' key in " +
			// getFileName());

			MAIN_COMMAND_ALIASES = keySet
					? getCommandList("Command_Aliases")
	...
	/**
	 * Inspect if some settings classes extend localization and make sure only
	 * one does, if any
	 *
	 * @return
	 */
	private static boolean hasLocalization() {
		...
		Valid.checkBoolean(localeClasses < 2,
				"You cannot have more than 1 class extend SimpleLocalization!");
	...
	protected final Config getConfigInstance() {
		...
		return new Config("Settings.yml",
				SimplePlugin.getData().getAbsolutePath());
	}
	...
	/**
	 * Set and update the config version automatically, however the
	 * {@link #VERSION} will contain the older version used in the file on the
	 * disk so you can use it for comparing in the init() methods
	 *
