...
public final class ChatPaginator {
	 ...
	 * and {@link org.mineacademy.bfo.settings.SimpleLocalization.Commands#HEADER_COLOR} for color.
	 */
	 ...
	 * The {@link org.mineacademy.bfo.settings.SimpleLocalization.Commands#HEADER_COLOR} color is used.
	 *
	 * @param linesPerPage
	 */
	...
	public void send(CommandSender sender, int page) {
		...
		for (final List<SimpleComponent> components : this.pages.values())
			for (final SimpleComponent component : components)
				component.replace("{count}", String.valueOf(amount++)).send(sender);
